...
class MeterView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        if (logic.meterValue != null) TopItemWidget2(logic: logic),
        if (logic.meterValue != null &&
            (logic.meterValue?.isWithin30Minutes ?? false))
          InterruptTip(value: (logic.meterValue?.statusUpdateTimeMill ?? 0)),
  ...
  Widget buildStatusItem(MonitorDetailV1Logic logic) => Column(
    children: [
      ...
      Container(
        ...
        child: Column(
          children: [
            if (logic.meterValue?.alarmStatus != null)
              Row(
                children: [
                  Text(
                    "${TKey.alarmStatus.tr}:",
                  ...
                  Spacer(),
                  Text(
                    logic.meterValue?.showAlarmStatus ?? "-",
                    style: TextStyle(
                      fontSize: 14,
                      color: (logic.meterValue?.alarmStatus == true)
                          ? Color(0xFFFF4C7B)
                          : Color(0xFF46F9EC),
                    ),
                  ),
            ...
            if (logic.meterValue?.communicationStatus != null)
              Row(
                children: [
                  Text(
                  ...
                  Spacer(),
                  Text(
                    logic.meterValue?.showCommunicationStatus ?? "-",
                    style: TextStyle(
                      fontSize: 14,
                      color: (logic.meterValue?.communicationStatus == true)
                          ? Color(0xFFFF4C7B)
                          : Color(0xFF46F9EC),
                    ),
                  ),
            ...
            if (logic.meterValue?.statusEnNameDesc != null)
              Row(
                children: [
                  Text(
                    "${TKey.runStatus.tr}:",
                  ...
                  Spacer(),
                  Text(
                    logic.meterValue?.showRunStatus ?? "-",
  ...
  Widget buildRealTimeData(MonitorDetailV1Logic logic) {
    return CustomScrollView(
      slivers: [
        ...
        if ((logic.meterValue?.list ?? []).isNotEmpty)
          SliverToBoxAdapter(
            child: Container(
              padding: EdgeInsetsDirectional.only(
                bottom: 16.h,
        ...
        if ((logic.meterValue?.list ?? []).isNotEmpty)
          SliverList.separated(
            itemCount: (logic.meterValue?.list ?? []).length,
            itemBuilder: (_, i) {
              ChildItemInfo item = (logic.meterValue?.list ?? [])[i];
              return Container(
                width: double.maxFinite,
