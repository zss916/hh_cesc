...
class DidoView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        if (logic.didoValue != null) TopItemWidget2(logic: logic),
        if (logic.didoValue != null &&
            (logic.didoValue?.isWithin30Minutes ?? false))
          InterruptTip(value: (logic.didoValue?.statusUpdateTimeMill ?? 0)),
  ...
  Widget buildRealTimeData(MonitorDetailV1Logic logic) {
    return CustomScrollView(
      slivers: [
        ...
        if ((logic.didoValue?.diList ?? []).isNotEmpty)
          SliverToBoxAdapter(
            child: Container(
              padding: EdgeInsetsDirectional.only(
                bottom: 16.h,
        ...
        if ((logic.didoValue?.doList ?? []).isNotEmpty)
          SliverToBoxAdapter(
            child: Container(
              padding: EdgeInsetsDirectional.only(
                bottom: 16.h,
