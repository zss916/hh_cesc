...
class CluView extends StatelessWidget {
  final MonitorDetailV1Logic logic;
  const CluView({super.key, required this.logic});

  @override
  Widget build(BuildContext context) {
    return Column(
      children: [
        if (logic.cluValue != null) TopItemWidget2(logic: logic),
        if (logic.cluValue != null &&
            (logic.cluValue?.isWithin30Minutes ?? false))
          InterruptTip(value: (logic.cluValue?.statusUpdateTimeMill ?? 0)),
  ...
  Widget buildStatusItem(MonitorDetailV1Logic logic) => Column(
    children: [
      ...
      Container(
        ...
        child: Column(
          children: [
            if (logic.cluValue?.alarmStatus != null)
              Row(
                children: [
                  Text(
                    "${TKey.alarmStatus.tr}:",
                  ...
                  Spacer(),
                  Text(
                    logic.cluValue?.showAlarmStatus ?? "-",
                    style: TextStyle(
                      fontSize: 14,
                      color: (logic.cluValue?.alarmStatus == true)
                          ? Color(0xFFFF4C7B)
                          : Color(0xFF46F9EC),
                    ),
                  ),
            ...
            if (logic.cluValue?.communicationStatus != null)
              Row(
                children: [
                  Text(
                  ...
                  Spacer(),
                  Text(
                    logic.cluValue?.showCommunicationStatus ?? "-",
                    style: TextStyle(
                      fontSize: 14,
                      color: (logic.cluValue?.communicationStatus == true)
                          ? Color(0xFFFF4C7B)
                          : Color(0xFF46F9EC),
                    ),
                  ),
            ...
            if (logic.cluValue?.statusEnNameDesc != null)
              Row(
                children: [
                  Text(
                    "${TKey.runStatus.tr}:",
  ...
  Widget buildRealTimeData(MonitorDetailV1Logic logic) {
    return CustomScrollView(
      slivers: [
        ...
        if ((logic.cluValue?.childList ?? []).isNotEmpty)
          SliverToBoxAdapter(
            child: Container(
              padding: EdgeInsetsDirectional.only(
                bottom: 16.h,
        ...
        if ((logic.cluValue?.childList ?? []).isNotEmpty)
          SliverList.separated(
            itemCount: (logic.cluValue?.childList ?? []).length,
            itemBuilder: (_, i) {
              ChildItemInfo item = (logic.cluValue?.childList ?? [])[i];
              return Container(
                width: double.maxFinite,
